...
function App() {
  const mySchedule = schedule(activities, 7);
        ...
        <Box display={'flex'} height={'100%'} justifyContent={'center'}>
          {
            mySchedule.map((day, index) => {
              return (
                <Box key={index} m={0.5} p={1} position={'relative'} sx={{
                  width: 200,
                  height: 2000,
                  background: '#706d6d',
                }}>
                    ...
                    (item, index) => {
                      return (
                        <Box key={index} mx={1} sx={{
                          ...
                          // width: 300,
                          minWidth: '70%',
                          minHeight: `${Math.max(100 * item.activity.duration / 1440 - 0.3, 0.7)}%`,
                          // top: 3.5 * item.time - 1400,
                          top: `${100 * item.time / 1440}%`,
                          left: index % 2 === 0 ? 0 : '',
                          right: index % 2 === 1 ? 0 : '',
                          boxShadow: 'rgba(0, 0, 0, 0.4) 0px 2px 4px, rgba(0, 0, 0, 0.3) 0px 7px 13px -10px, rgba(0, 0, 0, 0.2) 0px -3px 0px inset;'
                        }}>
